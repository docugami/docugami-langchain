...
class SQLResultChain(BaseDocugamiChain[dict]):
    ...
    fix_only_syntax_errors = False
    """Set to true to be more efficient and only try to fix SQL in case of errors."""

    def db_optimize(self) -> None:
        """Optimizes the database for few shot rows selection. This is optional
        but recommended. If not run, then random rows are selected from"""
        ...
