...
from docugami_langchain.output_parsers.soft_react_json_single_input import FINAL_ANSWER_ACTION
...
class BaseDocugamiAgent(BaseRunnable[T]):
    ...
    def format_chat_history(self, chat_history: list[tuple[str, str]]) -> list[BaseMessage]:
    ...
    def render_text_description(self, tools: list[BaseTool]) -> str:
        """
                ...
                for op in output.ops:
                    ...
                    if not final_streaming_started and op_path == "/streamed_output/-":
                        ...
                        if not len(op_value.keys()) == 1:
                            raise Exception("Expected output from one node at a time in step-wise agent streaming output")

                        key = list(op_value.keys())[0]
                        last_response_value = op_value[key]
                        yield TracedResponse[T](value=last_response_value)
                    elif op_path.startswith("/logs/") and op_path.endswith("/streamed_output/-"):
                        ...
                        if isinstance(op_value, AIMessageChunk):
                            ...
                            if not final_streaming_started:
                                ...
                                final_streaming_started = FINAL_ANSWER_ACTION in current_step_token_stream

                            if final_streaming_started:
                                # start streaming the final answer, we are done with intermediate steps
                                final_answer = str(current_step_token_stream).split(FINAL_ANSWER_ACTION)[-1].strip()
                                if final_answer:
                                    # start streaming the final answer, no more interim steps
                                    last_response_value = self.create_finish_state(final_answer)
                                    yield TracedResponse[T](value=last_response_value)
